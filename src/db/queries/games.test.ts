...
describe('logGame', () => {
  ...
  it('makes the next edit fork instead of amending', () => {
    ...
    logGame({ deckId, deckVersionId: built.versionId, result: 'loss' });
  ...
  it('leaves the lock timestamp alone on later matches', () => {
    ...
    logGame({ deckId, deckVersionId: versionId, result: 'loss' });
  ...
  it('keeps the format unrecorded when it was not asked', () => {
    ...
    expect(getGame(id)!.bestOf).toBeNull();
  });
  ...
  it('distinguishes on-the-draw from unknown', () => {
    const { deckId, versionId } = makeDeck();
    const drew = logGame({ deckId, deckVersionId: versionId, result: 'win', onPlay: false });
    const unknown = logGame({ deckId, deckVersionId: versionId, result: 'win' });

    // Recording "unknown" as false would bias the split rather than shrink it.
    expect(getGame(drew)!.onPlay).toBe(false);
    expect(getGame(unknown)!.onPlay).toBeNull();
  });
});
...
describe('transaction nesting', () => {
  /**
   * `logGame` and `saveDeckEdit` each open a transaction and neither may run
...
describe('deckRecord', () => {
  it('counts wins, losses and draws for a deck and for one version', () => {
    ...
    logGame({ deckId, deckVersionId: v2, result: 'draw' });
...
describe('versionGameCounts', () => {
  it('fills in now that the matches table exists', () => {
    ...
    expect(versionGameCounts(deckId).get(versionId)).toBe(2);
  });
  ...
  it('excludes soft-deleted matches', () => {
    ...
    const id = logGame({ deckId, deckVersionId: versionId, result: 'win' });
    deleteGame(id);
    expect(versionGameCounts(deckId).get(versionId)).toBeUndefined();
  });
});
...
describe('undo and delete', () => {
  it('undo removes the row entirely, leaving nothing to sync', () => {
    ...
    expect(
      db.getFirstSync<{ n: number }>('SELECT COUNT(*) AS n FROM games WHERE id = ?', [id])!.n
    ).toBe(0);
    expect(deckRecord(deckId).total).toBe(0);
  });
  ...
  it('delete keeps a tombstone so the deletion propagates', () => {
    ...
    const raw = db.getFirstSync<{ deleted_at: string | null; dirty: number }>(
      'SELECT deleted_at, dirty FROM games WHERE id = ?',
      [id]
    );
    expect(raw?.deleted_at).toBeTruthy();
    expect(raw?.dirty).toBe(1);
    expect(getGame(id)).toBeNull();
  });
  ...
  it('keeps the lock when another match still references the version', () => {
    const { deckId, versionId } = makeDeck();
    logGame({ deckId, deckVersionId: versionId, result: 'win' });
    const second = logGame({ deckId, deckVersionId: versionId, result: 'loss' });
  ...
  it('keeps the lock when a real match was played and then deleted', () => {
    const { deckId, versionId } = makeDeck();
    const played = logGame({ deckId, deckVersionId: versionId, result: 'win' });
    const mistap = logGame({ deckId, deckVersionId: versionId, result: 'loss' });

    deleteGame(played); // a real game, removed — leaves a tombstone
    undoGame(mistap); // a mis-tap, erased
...
describe('listGames and updateGame', () => {
  it('lists newest first and scopes to a deck', () => {
    ...
    expect(listGames().map((m) => m.result)).toEqual(['loss', 'draw', 'win']);
    expect(listGames({ deckId: first.deckId })).toHaveLength(2);
    expect(listGames({ limit: 1 })).toHaveLength(1);
  });
  ...
  it('can clear an optional field back to null', () => {
    const { deckId, versionId } = makeDeck();
    const id = logGame({ deckId, deckVersionId: versionId, result: 'win', onPlay: true });

    updateGame(id, { onPlay: null });
    expect(getGame(id)!.onPlay).toBeNull();
  });
});
